...
import { 
  ...
  ChevronLeft, ChevronRight, ArrowLeft, X, GraduationCap
} from 'lucide-react';
...
const searchCursos = (cursos: Curso[], query: string): Curso[] => {
  ...
  return cursos.filter(c => 
    ...
    (c.det_descripcion?.toLowerCase().includes(safeQuery) || false)
  );
};
...
function CursosContent() {
  ...
  const cursosFiltrados = useMemo(() => {
    ...
    if (tipoActivo !== 'TODOS') {
      filtrados = filtrados.filter(c => c.tipo_curso_otro?.tipo_conv_curso_nombre?.toUpperCase() === tipoActivo);
    }
  ...
  const getColorClasses = (tipo: string) => {
    ...
    if (tipoUpper === 'CURSOS' || tipoUpper === 'PRESENCIAL') {
      return { bg: `${hexToRgba(safePrimary, 0.15)}`, border: `${hexToRgba(safePrimary, 0.3)}`, text: safePrimary };
    }
    if (tipoUpper === 'SEMINARIOS') {
      return { bg: `${hexToRgba(safeSecondary, 0.15)}`, border: `${hexToRgba(safeSecondary, 0.3)}`, text: safeSecondary };
    }
    if (tipoUpper === 'TALLERES') {
      return { bg: `${hexToRgba(safeTertiary, 0.15)}`, border: `${hexToRgba(safeTertiary, 0.3)}`, text: safeTertiary };
    }
      ...
      <div className="min-h-screen flex flex-col" style={{ background: `linear-gradient(135deg, ${hexToRgba(primaryColor, 0.1)}, ${hexToRgba(secondaryColor, 0.1)})` }}>
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="text-center max-w-md">
            ...
            <h2 className="text-3xl font-bold mb-3 text-gray-900">Error al cargar cursos</h2>
         ...
         <div className="absolute inset-0 opacity-70" style={{ background: `linear-gradient(135deg, ${primaryColor} 0%, ${secondaryColor} 100%)` }} /> 
          ...
          <div className="relative max-w-6xl mx-auto px-4">
            ...
            <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-white/15 backdrop-blur-sm border border-white/20 mb-8">
              ...
              <span className="text-sm text-white font-medium">{cursos.length} cursos disponibles</span>
            </div>
              ...
              <div className="mt-3 flex items-center justify-between text-sm">
                <span className="text-white/80">
                  {cursosFiltrados.length > 0 
                    ...
                    : busqueda ? 'Sin resultados' : `${cursos.length} cursos totales`
                  }
                </span>
                {busqueda && (
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                  tipoActivo === 'TODOS' ? 'text-white shadow-md scale-105' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
                  ...
                  <>
                    <div className="grid md:grid-cols-2 gap-6">
                      {cursosPagina.map((curso) => {
                        ...
                        return (
                          <Link key={curso.iddetalle_cursos_academicos} href={`/cursos/${curso.iddetalle_cursos_academicos}`} className="group">
                            <div className="bg-white rounded-2xl overflow-hidden border shadow-sm hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 h-full flex flex-col" style={{ borderColor: colors.border }}>
                              ...
                              <div className="p-6 flex-1 flex flex-col">
                                ...
                                {curso.det_descripcion && (
                                  <p className="text-sm text-gray-600 mb-4 line-clamp-3 leading-relaxed flex-1" dangerouslySetInnerHTML={{ __html: sanitizeHTML(curso.det_descripcion) }} />
                                )}
                                ...
                                <div className="space-y-2 pt-4 border-t" style={{ borderColor: `${hexToRgba(primaryColor, 0.15)}` }}>
                                  <div className="flex items-center gap-2 text-xs">
                                    ...
                                    <span className="text-gray-600">{curso.det_carga_horaria || 'Por definir'} horas</span>
                                  </div>
                                  <div className="flex items-center gap-2 text-xs">
                                    <Users className="w-4 h-4" style={{ color: colors.text }} />
                                    <span className="text-gray-600">{curso.det_cupo_max} cupos</span>
                                  </div>
                                  {curso.det_costo > 0 && (
                    ...
                    <p className="text-center text-sm mt-6" style={{ color: `${hexToRgba(primaryColor, 0.7)}` }}>
                      Página {safePaginaActual} de {totalPaginas} - Mostrando {cursosPagina.length} de {cursosFiltrados.length} cursos
                    </p>
                  </>
                )}
              </div>
                ...
                <div className="sticky top-24 space-y-8">
                  ...
                  {eventos.length > 0 && (
                    <div className="bg-white rounded-2xl border shadow-lg overflow-hidden" style={{ borderColor: `${hexToRgba(secondaryColor, 0.2)}` }}>
                      <div className="p-4 border-b" style={{ borderColor: `${hexToRgba(secondaryColor, 0.2)}`, background: `${hexToRgba(secondaryColor, 0.05)}` }}>
                        <h3 className="font-bold text-lg text-gray-900">Próximos Eventos</h3>
                      </div>
                      <div className="p-4 space-y-3">
                        {eventos.slice(0, 3).map((evento) => (
                          <div key={evento.evento_id} className="p-4 rounded-xl border hover:shadow-md transition-all hover:-translate-y-0.5" style={{ borderColor: `${hexToRgba(primaryColor, 0.15)}`, background: `${hexToRgba(primaryColor, 0.03)}` }}>
                            <h4 className="font-semibold text-sm mb-2 line-clamp-2 text-gray-900">
                              {sanitizeText(evento.evento_titulo, 80)}
                            </h4>
                            <div className="flex items-center gap-2 text-xs text-gray-600">
                              <CalendarIcon className="w-3 h-3" style={{ color: primaryColor }} />
