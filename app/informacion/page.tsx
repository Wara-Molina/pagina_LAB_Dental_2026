...
const SECCIONES_VALIDAS: SeccionInfo[] = [
  "mision-vision",
  "autoridades",
  "historia",
  "ubicacion",
...
const isValidExternalUrl = (url: string | undefined): boolean => {
  ...
  try {
    ...
    const safeDomain = safeDomains.some((domain) =>
...
function InformacionContent() {
  ...
  const [seccionActiva, setSeccionActiva] =
    useState<SeccionInfo>(initialSeccion);
  ...
  const secciones: Array<{
  ...
  }> = [
    {
      ...
      icon: Target,
      description: "Nuestro propósito y proyección",
    },
    {
      id: "autoridades",
      label: "Autoridades",
    ...
    const fetchData = async () => {
      try {
        ...
        setInstitucion({
          ...
          institucion_historia: sanitizeTextField(
            instData.institucion_historia,
          ),
          institucion_objetivos: sanitizeTextField(
            instData.institucion_objetivos,
          ),
          institucion_direccion: sanitizeTextField(
            instData.institucion_direccion,
            300,
          ),
          institucion_correo1: instData.institucion_correo1?.replace(
            /[<>\"'&]/g,
            "",
          ),
        });

        const autoridadesSanitizadas = (contenidoRes.data.autoridad || []).map(
          (a: any) => ({
            ...a,
            nombre_autoridad: sanitizeTextField(a.nombre_autoridad, 100),
            cargo_autoridad: sanitizeTextField(a.cargo_autoridad, 100),
            facebook_autoridad: isValidExternalUrl(a.facebook_autoridad)
              ? a.facebook_autoridad
              : undefined,
            twiter_autoridad: isValidExternalUrl(a.twiter_autoridad)
              ? a.twiter_autoridad
              : undefined,
          }),
        );
        ...
        setUbicacion(
          ubicacionData
            ? {
                ...ubicacionData,
                ubicacion_titulo: sanitizeTextField(
                  ubicacionData.ubicacion_titulo,
                  100,
                ),
                ubicacion_descripcion: sanitizeTextField(
                  ubicacionData.ubicacion_descripcion,
                ),
                ubicacion_latitud: ubicacionData.ubicacion_latitud?.replace(
                  /[<>\"'&]/g,
                  "",
                ),
                ubicacion_longitud: ubicacionData.ubicacion_longitud?.replace(
                  /[<>\"'&]/g,
                  "",
                ),
              }
        ...
        <section className="relative py-20 lg:py-28 overflow-hidden">
          ...
          <div className="relative max-w-6xl mx-auto px-4">
            ...
            <p className="text-lg md:text-xl text-white/90 max-w-3xl leading-relaxed mb-8">
              Conoce nuestra misión, visión, historia, autoridades y ubicación
              de{" "}
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {/* Mission */}
        <div
          className="group bg-white rounded-2xl overflow-hidden border shadow-lg hover:shadow-2xl transition-all duration-300"
          style={{ borderColor: `${hexToRgba(primaryColor, 0.2)}` }}
        >
          <div
            className="p-1"
            style={{
              background: `linear-gradient(135deg, ${hexToRgba(primaryColor, 0.3)}, ${hexToRgba(primaryColor, 0.1)})`,
            }}
          >
                ...
                dangerouslySetInnerHTML={{
                  __html: sanitizeHTML(
                    institucion?.institucion_mision ||
                      "<p>Formar profesionales competentes con enfoque holístico.</p>",
                  ),
                }}
              />
            </div>
        ...
        >
          <div
            className="p-1"
            style={{
              background: `linear-gradient(135deg, ${hexToRgba(secondaryColor, 0.3)}, ${hexToRgba(secondaryColor, 0.1)})`,
            }}
          >
            <div className="bg-white rounded-xl p-8 h-full">
              <div className="flex items-center gap-4 mb-6">
                ...
                dangerouslySetInnerHTML={{
                  __html: sanitizeHTML(
                    institucion?.institucion_vision ||
                      "<p>Ser referentes en educación superior.</p>",
                  ),
                }}
              />
            </div>
      ...
      {institucion?.institucion_objetivos && (
        ...
        >
          <div
            className="p-1"
            style={{
              background: `linear-gradient(90deg, ${hexToRgba(primaryColor, 0.2)}, transparent)`,
            }}
          >
            <div className="p-8">
              ...
              <div
                className="text-gray-700 leading-relaxed prose prose-sm max-w-none"
                dangerouslySetInnerHTML={{
                  __html: sanitizeHTML(institucion.institucion_objetivos),
                }}
              />
            </div>
          </div>
        ...
        {[
          {
            icon: Users,
            title: "Compromiso",
            desc: "Dedicación total a la excelencia educativa",
            color: primaryColor,
          },
          {
            icon: BookOpen,
            title: "Calidad",
            desc: "Estándares académicos de excelencia",
            color: secondaryColor,
          },
          {
            icon: Target,
            title: "Innovación",
            desc: "Metodologías educativas modernas",
            color: tertiaryColor,
          },
          {
            icon: TrendingUp,
          ...
          <div
            key={idx}
            className="text-center p-6 rounded-2xl bg-white border shadow-sm hover:shadow-lg transition-all hover:-translate-y-1 group"
            style={{ borderColor: `${hexToRgba(item.color, 0.2)}` }}
          >
            <div
    ...
    <div>
      {autoridades.length > 0 ? (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {autoridades.map((autoridad) => (
            <div
              ...
              className="group bg-white rounded-2xl overflow-hidden border shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2"
              style={{ borderColor: `${hexToRgba(primaryColor, 0.2)}` }}
            >
              {/* Foto */}
                ...
                >
                  ...
                  {(autoridad.facebook_autoridad ||
                    ...
                    <div className="flex gap-2 pt-2">
                      {autoridad.facebook_autoridad &&
                        isValidExternalUrl(autoridad.facebook_autoridad) && (
                          <a
                            href={autoridad.facebook_autoridad}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="p-2.5 rounded-xl bg-gray-100 hover:bg-blue-50 transition-colors"
                            style={{ color: "#1877F2" }}
                            aria-label={`Facebook de ${autoridad.nombre_autoridad}`}
                          >
                            <Facebook className="w-4 h-4" />
                          </a>
                        )}
                      {autoridad.twiter_autoridad &&
                        isValidExternalUrl(autoridad.twiter_autoridad) && (
                          <a
                            ...
                            className="p-2.5 rounded-xl bg-gray-100 hover:bg-blue-50 transition-colors"
                            style={{ color: "#0088cc" }}
                            aria-label={`Twitter de ${autoridad.nombre_autoridad}`}
                          >
        ...
        <div className="text-center py-20">
          ...
          <Link
            href="/contacto"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-white font-medium shadow-lg hover:shadow-xl transition-all"
          ...
          <div className="p-8">
            ...
            {institucion?.institucion_historia ? (
              <div
                className="text-gray-700 leading-relaxed prose prose-sm max-w-none"
                dangerouslySetInnerHTML={{
                  __html: sanitizeHTML(institucion.institucion_historia),
                }}
              />
            ) : (
              <>
                <p className="text-gray-700 leading-relaxed mb-4">
                  Somos una institución comprometida con la excelencia
                  académica.
                </p>
                <p className="text-gray-700 leading-relaxed">
                  Nuestra trayectoria se caracteriza por la innovación
                  pedagógica.
                </p>
              </>
            )}
      ...
      {institucion?.institucion_objetivos ? (
        ...
        >
          <div
            className="p-1"
            style={{
              background: `linear-gradient(90deg, ${hexToRgba(secondaryColor, 0.2)}, transparent)`,
            }}
          >
            <div className="p-8">
              <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2
            className="text-3xl font-bold text-center mb-10 font-serif"
            style={{ color: primaryColor }}
          >
            ...
            {[
              {
                ...
                desc: "Servicio a la comunidad",
                color: primaryColor,
              },
              {
                icon: BookOpen,
                title: "Excelencia Académica",
                desc: "Formación de calidad",
                color: secondaryColor,
              },
              {
                icon: TrendingUp,
                title: "Innovación",
                desc: "Nuevas tecnologías",
                color: tertiaryColor,
              },
              {
                icon: Award,
              ...
              <div
                key={idx}
                className="bg-white p-6 rounded-2xl border shadow-sm hover:shadow-lg transition-all hover:-translate-y-1 text-center group"
                style={{ borderColor: `${hexToRgba(valor.color, 0.2)}` }}
              >
                <div
                  className="w-14 h-14 rounded-xl flex items-center justify-center mx-auto mb-4 transition-transform group-hover:scale-110"
                  style={{ backgroundColor: `${hexToRgba(valor.color, 0.15)}` }}
                >
                  <valor.icon
                    className="w-7 h-7"
...
function SeccionUbicacion({
  ...
  const safeMapUrl = useMemo(() => {
    return validateGoogleMapsUrl(institucion?.institucion_api_google_map) || "";
  }, [institucion?.institucion_api_google_map]);

  const safeCoords = useMemo(() => {
    const lat =
      ubicacion?.ubicacion_latitud?.replace(/[^0-9.\-]/g, "") ||
      "-16.489549430458553";
    const lng =
      ubicacion?.ubicacion_longitud?.replace(/[^0-9.\-]/g, "") ||
      "-68.19329917301572";
    return { lat, lng };
            ...
            >
              <div className="p-8">
                ...
                <p
                  ...
                  dangerouslySetInnerHTML={{
                    __html: sanitizeHTML(ubicacion.ubicacion_descripcion),
                  }}
                />
              </div>
            </div>
