...
import { 
  ...
  Download, ChevronLeft, ChevronRight, FileText, X
} from 'lucide-react';
...
const searchPublicaciones = (publicaciones: Publicacion[], query: string): Publicacion[] => {
  ...
  return publicaciones.filter(p => 
    ...
    (p.publicaciones_descripcion?.toLowerCase().includes(safeQuery) || false) ||
    (p.publicaciones_autor?.toLowerCase().includes(safeQuery) || false)
  );
};
        ...
        <section className="relative py-20 lg:py-28 overflow-hidden">
          ...
          <div className="relative max-w-6xl mx-auto px-4">
            ...
            <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-white/15 backdrop-blur-sm border border-white/20 mb-8">
              ...
              <span className="text-sm text-white font-medium">{publicaciones.length} publicaciones disponibles</span>
            </div>
              ...
              <div className="mt-3 flex items-center justify-between text-sm">
                <span className="text-white/80">
                  {publicacionesFiltradas.length > 0 
                    ...
                    : busqueda ? 'Sin resultados' : `${publicaciones.length} publicaciones totales`
                  }
                </span>
                {busqueda && (
              ...
              {categorias.map((categoria) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                    categoriaActiva === categoria ? 'text-white shadow-md scale-105' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {publicacionesPagina.length === 0 ? (
              <div className="text-center py-20">
                ...
                <h3 className="text-2xl font-bold mb-2 text-gray-900">No se encontraron publicaciones</h3>
              ...
              <>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-12">
                  {publicacionesPagina.map((publicacion) => (
                    <Link key={publicacion.publicaciones_id} href={`/publicaciones/${publicacion.publicaciones_id}`} className="group">
                      <div className="bg-white rounded-2xl overflow-hidden border shadow-sm hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 h-full flex flex-col" style={{ borderColor: `${hexToRgba(primaryColor, 0.2)}` }}>
                        ...
                        <div className="relative h-48 overflow-hidden bg-gray-100">
                          ...
                          {publicacion.publicaciones_tipo && (
                            <div className="absolute top-3 left-3">
                              <span className="px-3 py-1.5 rounded-full text-xs font-semibold bg-white/95 backdrop-blur-sm shadow-lg" style={{ color: primaryColor }}>
                                {publicacion.publicaciones_tipo}
                              </span>
                            </div>
                          )}
                        ...
                        <div className="p-6 flex-1 flex flex-col">
                          ...
                          {publicacion.publicaciones_descripcion && (
                            <p className="text-sm text-gray-600 mb-4 line-clamp-3 leading-relaxed flex-1" dangerouslySetInnerHTML={{ __html: sanitizeHTML(publicacion.publicaciones_descripcion) }} />
                          )}
                          ...
                          <div className="space-y-2 pt-4 border-t" style={{ borderColor: `${hexToRgba(primaryColor, 0.15)}` }}>
                            {publicacion.publicaciones_autor && (
                              <div className="flex items-center gap-2 text-xs">
                                ...
                                <span className="text-gray-600 line-clamp-1">{publicacion.publicaciones_autor}</span>
                              </div>
                            )}
                            <div className="flex items-center gap-2 text-xs">
                              <Calendar className="w-4 h-4" style={{ color: primaryColor }} />
                              <span className="text-gray-600">{formatDate(publicacion.publicaciones_fecha)}</span>
                            </div>
                          </div>
        ...
        <section className="py-16" style={{ background: `linear-gradient(135deg, ${hexToRgba(primaryColor, 0.1)}, ${hexToRgba(secondaryColor, 0.1)})` }}>
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            <p className="text-gray-600 mb-8 text-lg">
              Contáctanos y te ayudaremos a encontrar la publicación que necesitas
            </p>
            <Link 
              href="/contacto"
