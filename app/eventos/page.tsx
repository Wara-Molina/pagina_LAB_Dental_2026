...
const searchEventos = (eventos: Evento[], query: string): Evento[] => {
  ...
  return eventos.filter(e => 
    ...
    (e.evento_descripcion?.toLowerCase().includes(safeQuery) || false) ||
    e.tipo_evento.toLowerCase().includes(safeQuery) ||
    (e.evento_lugar?.toLowerCase().includes(safeQuery) || false)
  );
};
...
function EventosContent() {
  ...
  const getTypeStyle = (type: string) => {
    ...
    if (t.includes('TALLER') || t.includes('WORKSHOP')) return { backgroundColor: `${hexToRgba(safeSecondary, 0.15)}`, color: safeSecondary };
    if (t.includes('SEMINARIO')) return { backgroundColor: `${hexToRgba('#f59e0b', 0.15)}`, color: '#f59e0b' };
         ...
         <div className="absolute inset-0 opacity-70" style={{ background: `linear-gradient(135deg, ${primaryColor} 0%, ${secondaryColor} 100%)` }} /> 
          ...
          <div className="relative max-w-6xl mx-auto px-4">
            ...
            <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-white/15 backdrop-blur-sm border border-white/20 mb-8">
              ...
              <span className="text-sm text-white font-medium">{eventos.length} eventos disponibles</span>
            </div>
              ...
              <div className="mt-3 flex items-center justify-between text-sm">
                <span className="text-white/80">
                  {eventosFiltrados.length > 0 
                    ...
                    : busqueda ? 'Sin resultados' : `${eventos.length} eventos totales`
                  }
                </span>
                {busqueda && (
                          ...
                          <div className="bg-white rounded-2xl overflow-hidden border shadow-sm hover:shadow-2xl transition-all duration-300 hover:-translate-y-2" style={{ borderColor: `${hexToRgba(primaryColor, 0.2)}` }}>
                            <div className="p-6 flex flex-col md:flex-row md:items-start gap-6">
                              ...
                              <div className="flex-shrink-0">
                                <div className="rounded-xl p-4 text-center w-20 shadow-sm" style={{ backgroundColor: `${hexToRgba(primaryColor, 0.15)}` }}>
                                  <div className="text-xs uppercase font-semibold" style={{ color: `${hexToRgba(primaryColor, 0.8)}` }}>
                                    {new Date(event.evento_fecha).toLocaleDateString('es-ES', { month: 'short' })}
                                  </div>
                                  <div className="text-2xl font-bold" style={{ color: primaryColor }}>
                                    {new Date(event.evento_fecha).getDate()}
                                  </div>
                                  <div className="text-xs" style={{ color: `${hexToRgba(primaryColor, 0.7)}` }}>
                              ...
                              <div className="flex-1">
                                <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-3">
                                  ...
                                  <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold w-fit" style={getTypeStyle(event.tipo_evento)}>
                                    {event.tipo_evento}
                                  </span>
                                </div>

                                {event.evento_descripcion && (
                                  <p className="text-gray-600 mb-4 line-clamp-2 leading-relaxed" dangerouslySetInnerHTML={{ __html: sanitizeHTML(event.evento_descripcion) }} />
                                )}

                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t" style={{ borderColor: `${hexToRgba(primaryColor, 0.15)}` }}>
                                  <div className="flex items-center gap-2 text-sm">
                                    <Clock className="w-4 h-4" style={{ color: primaryColor }} />
                                    <span className="text-gray-600">{event.evento_hora ? event.evento_hora.substring(0, 5) : 'Hora por confirmar'}</span>
                                  </div>
                                  <div className="flex items-center gap-2 text-sm">
                                    <MapPin className="w-4 h-4" style={{ color: primaryColor }} />
                                    <span className="text-gray-600 truncate">{event.evento_lugar || 'Lugar por confirmar'}</span>
                                  </div>
                                  <div className="flex items-center gap-2 text-sm">
                                    <Calendar className="w-4 h-4" style={{ color: primaryColor }} />
                                    <span className="text-gray-600">{formatDateFull(event.evento_fecha)}</span>
                                  </div>
                                </div>
                              </div>
                            </div>
                      ...
                      <div className="p-4 space-y-3">
                        {eventos.slice(0, 4).map((evento) => (
                          <Link key={evento.evento_id} href={`/eventos/${evento.evento_id}`} className="block group">
                            <div className="p-4 rounded-xl border hover:shadow-md transition-all hover:-translate-y-0.5" style={{ borderColor: `${hexToRgba(primaryColor, 0.15)}`, background: `${hexToRgba(primaryColor, 0.03)}` }}>
