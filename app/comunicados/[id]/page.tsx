...
import { 
  ...
  ExternalLink, Bell, X, ZoomIn, Mail, Printer
} from 'lucide-react';
...
function ComunicadoDetalleContent() {
  ...
  const getTipoColor = () => {
    ...
    const safeSecondary = getSafeColor("white", '#ffffff');
    
    if (tipo === 'CONVOCATORIAS') return { bg: `${hexToRgba(safePrimary, 0.15)}`, text: safePrimary };
    if (tipo === 'AVISOS') return { bg: `${hexToRgba('#f59e0b', 0.15)}`, text: '#ffffff' };
    if (tipo === 'COMUNICADOS') return { bg: `${hexToRgba(safeSecondary, 0.15)}`, text: safeSecondary };
  ...
  const imageUrl = useMemo(() => {
    if (!comunicado?.con_foto_portada) return '';
    const url = getStorageUrl(comunicado.con_foto_portada);
      ...
      <div className="min-h-screen flex flex-col" style={{ background: `linear-gradient(135deg, ${hexToRgba(primaryColor, 0.1)}, ${hexToRgba(secondaryColor, 0.1)})` }}>
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="text-center max-w-md">
            ...
            <h2 className="text-3xl font-bold mb-3 text-gray-900">{error || 'Comunicado no encontrado'}</h2>
            <p className="text-gray-600 mb-8">El comunicado que buscas no existe o ha sido eliminado</p>
            <div className="flex gap-4 justify-center">
              <Link href="/comunicados" className="inline-flex items-center gap-2 px-8 py-3 rounded-full font-medium text-white shadow-lg hover:shadow-xl transition-all" style={{ backgroundColor: primaryColor }}>
                <ArrowLeft className="w-5 h-5" /> Volver a comunicados
              </Link>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: `linear-gradient(180deg, #fff 0%, ${hexToRgba(primaryColor, 0.08)} 100%)` }}>
      ...
      <main className="flex-1">
        {/* Hero Image */}
        {comunicado.con_foto_portada && imageUrl ? (
...
>
  ...
  <br /><br />
            ...
            <div className="max-w-6xl mx-auto px-4">
              ...
              <div className="flex items-center gap-4 mb-4">
                <div className="p-3 rounded-xl" style={{ backgroundColor: `${hexToRgba(primaryColor, 0.15)}` }}>
                  {tipoLabel === 'CONVOCATORIAS' ? <Calendar className="w-7 h-7" style={{ color: primaryColor }} /> :
                   tipoLabel === 'AVISOS' ? <Bell className="w-7 h-7" style={{ color: primaryColor }} /> :
                   <FileText className="w-7 h-7" style={{ color: primaryColor }} />}
                </div>
                <h1 className="text-3xl md:text-4xl font-bold text-gray-900 font-serif">{sanitizeText(comunicado.con_titulo, 150)}</h1>
              </div>
              <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold" style={{ backgroundColor: colors.bg, color: colors.text }}>
                {tipoDisplay}
              </span>
            </div>
          </div>
        ...
        {/* Image Modal */}
        {imageModalOpen && comunicado.con_foto_portada && imageUrl && (
          <div 
          ...
          >
            ...
            <div className="relative w-full h-full flex items-center justify-center p-4 sm:p-8" onClick={(e) => e.stopPropagation()}>
              <div className="max-w-lg h-auto object-contain rounded-lg shadow-2xl mx-auto">
                <Image 
                  src={imageUrl} 
                  alt={sanitizeText(comunicado.con_titulo, 150)} 
                  width={1920} 
              ...
              <div className="bg-white rounded-3xl shadow-xl border p-6 md:p-8" style={{ borderColor: `${hexToRgba(primaryColor, 0.2)}` }}>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4 mb-8">
                    {comunicado.con_fecha_inicio && (
                      <div className="p-5 rounded-2xl border text-center" style={{ borderColor: `${hexToRgba(primaryColor, 0.2)}`, background: `${hexToRgba(primaryColor, 0.05)}` }}>
                        ...
                        <p className="text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: `${hexToRgba(primaryColor, 0.7)}` }}>Fecha de inicio</p>
                        <p className="font-medium text-gray-900">{formatDate(comunicado.con_fecha_inicio)}</p>
                      </div>
                    )}
                    ...
                    {comunicado.con_fecha_fin && (
                      <div className="p-5 rounded-2xl border text-center" style={{ borderColor: `${hexToRgba(secondaryColor, 0.2)}`, background: `${hexToRgba(secondaryColor, 0.05)}` }}>
                        ...
                        <p className="text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: `${hexToRgba(secondaryColor, 0.7)}` }}>Fecha de fin</p>
                        <p className="font-medium text-gray-900">{formatDate(comunicado.con_fecha_fin)}</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                {comunicado.con_descripcion && (
                  <div className="mb-10">
                    ...
                    <div className="prose prose-lg max-w-none text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: sanitizeHTML(comunicado.con_descripcion) }} />
                  </div>
                )}
              ...
              <div className="bg-white rounded-3xl shadow-lg border p-6 lg:sticky lg:top-24" style={{ borderColor: `${hexToRgba(primaryColor, 0.2)}` }}>
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <p className="font-semibold text-gray-900">{tipoDisplay}</p>
                  </div>
