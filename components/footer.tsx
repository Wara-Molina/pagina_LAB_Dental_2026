...
export function Footer() {
  ...
  useEffect(() => {
    ...
    fetchData();
    return () => { isMounted = false; }; // ✅ Cleanup para evitar setState en componente desmontado
  }, [institucionId]);
  ...
  const hoverText = isLightBackground ? 'hover:text-gray-900' : 'hover:text-white';
  ...
  const socialLinks = [
    ...
    { name: 'Telegram', url: institucion?.institucion_twitter, icon: Twitter, color: '#0088cc' },
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-8 mb-16">
          ...
          <div className="lg:col-span-2">
            <Link href="/" className="flex items-center gap-3 mb-6 group">
              ...
              <div>
                <span className={`font-serif text-lg font-medium ${textColor} group-hover:opacity-90 transition-opacity`}>
                  {institucionNombre}
                </span>
                <p className={`text-xs ${textColorDimmed}`}>{institucionIniciales}</p>
              </div>
            </Link>
            
            {/* Misión sanitizada */}
            <p className={`${textColorMuted} leading-relaxed mb-6 max-w-sm text-sm`}>
            ...
            <div className="space-y-3 text-sm">
              {institucion?.institucion_correo1 && (
                ...
                >
                  ...
                  <span className="truncate">{institucion.institucion_correo1}</span>
                </a>
              )}
              {(institucion?.institucion_celular1 || institucion?.institucion_celular2) && (
                ...
                >
                  ...
                  <span>+591 {institucion.institucion_celular1 || institucion.institucion_celular2}</span>
                </a>
              )}
              {institucion?.institucion_direccion && (
                <div className={`flex items-start gap-3 ${textColorMuted}`}>
                  <MapPin className="w-4 h-4 flex-shrink-0 mt-0.5" />
                  <span className="text-sm">{institucion.institucion_direccion}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {linksExternos.length > 0 ? (
                ...
                <li className={`text-sm ${textColorDimmed}`}>Sin enlaces disponibles</li>
              )}
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-3 mb-6">
              ...
              {socialLinks.length === 0 && (
                <span className={`text-xs ${textColorDimmed}`}>Sin redes sociales</span>
              )}
            </div>
            ...
            <div className={`pt-6 border-t ${borderColor}`}>
              <div className="flex items-center gap-3">
                <div className={`relative w-12 h-12 rounded-lg overflow-hidden ${isLightBackground ? 'bg-gray-200' : 'bg-white'} p-1`}>
                  {logoUrl ? (
                    <Image
                      src={logoUrl}
                    ...
                    <GraduationCap className={`w-6 h-6 ${textColorDimmed} m-3`} />
                  )}
                </div>
                <div>
                  <p className={`text-xs font-medium ${textColor}`}>{institucionNombre}</p>
                  <p className={`text-xs ${textColorFaint}`}>{institucionIniciales}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className={`border-t ${borderColor} pt-8 flex flex-col md:flex-row justify-between items-center gap-4`}>
          <p className={`text-sm ${textColorFaint} text-center md:text-left`}>
            &copy; {new Date().getFullYear()} {institucionNombre}. Todos los derechos reservados.
          </p>
          
          <div className="flex gap-6 text-sm">
            <Link href="/privacidad" className={`${textColorFaint} ${hoverText} transition-colors`}>
              Política de privacidad
            </Link>
            <Link href="/terminos" className={`${textColorFaint} ${hoverText} transition-colors`}>
              Términos de uso
            </Link>
            <a 
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={`${textColorFaint} ${hoverText} transition-colors flex items-center gap-1`}
            >
